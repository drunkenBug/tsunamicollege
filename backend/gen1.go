...
func generateQuestions(X []int, Y []int, op operator) []db.Question {
	...
	for _, x := range X {
		for _, y := range Y {
			...
			next := db.Question{Title: title, Solution: solution, TopicTitle: op.title, Rating: x+y}
			arr = append(arr, next)
		}
	}
	return arr
...
func generateDivisions(Y []int, Z []int, op operator) []db.Question {
	...
	for _, y := range Y {
		for _, z := range Z {
			...
			next := db.Question{Title: title, Solution: solution, TopicTitle: op.title, Rating: y+z}
			arr = append(arr, next)
		}
	}
	return arr
